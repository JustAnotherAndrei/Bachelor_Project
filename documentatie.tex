...
optical-fibre attenuation model, an adaptive (Smart) Eve, and an ML-based
detection layer into the same code base without restructuring. Further
extensions identified for the present cycle and discussed in
Section~\ref{sec:conclusions} include alternative QKD protocols (E91, B92,
SARG04), an explicit photon-number-splitting attack, finite-key security
bounds, and a single-player gamified mode in which the user plays the role
of Eve and is scored against the ML detector.
...
The literature has begun to explore deeper models for this task
recently~\cite{xu2024dlqkd,zhao2025rlqkd}. Deep recurrent networks
(LSTM) can in principle ingest the entire per-qubit error history rather
than aggregate features, and have been shown to reach $>90\%$ accuracy
against adaptive attackers in CV-QKD~\cite{xu2024dlqkd}. The choice of a
Random Forest in Sequre is a deliberate trade-off: it provides
near-state-of-the-art accuracy on aggregate-feature classification with
a fraction of the data, the training, and the interpretability cost of a
deep model, while leaving the LSTM extension as an explicit future
direction (see Section~\ref{sec:conclusions}).
