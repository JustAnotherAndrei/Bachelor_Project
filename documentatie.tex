...
\noindent
Quantum Key Distribution (QKD) promises information-theoretic security by
encoding cryptographic key material into single photons whose state cannot
be cloned or measured without disturbance. Despite this strong theoretical
foundation, hands-on QKD experimentation is still difficult to access:
existing simulators are either research-grade command-line tools written
for specialists, or oversimplified notebooks that hide the very details
(noise, finite-key effects, source imperfections, detector inefficiency,
realistic attack strategies) that make QKD interesting and non-trivial.

This thesis presents \textbf{Sequre}, an interactive, full-stack platform
for studying and visualising QKD. The system implements four protocols
(BB84, B92, SARG04 and the entanglement-based E91) on top of a unified
Qiskit~\cite{qiskit2024} simulation backend, and also dispatches the same
circuits to real superconducting hardware via the IBM Quantum runtime.
The end-to-end pipeline is faithfully modelled: weak-coherent-pulse
sources with three-intensity decoy-state analysis, photon-number-splitting
and intercept-resend eavesdropping, depolarising and measurement noise,
fibre attenuation at $0.20$\,dB/km, detector efficiency and dark counts,
Cascade-style information reconciliation, Toeplitz-hash privacy
amplification, finite-key correction in the Tomamichel--Lim--Gisin--Renner
framework, and CHSH Bell tests for E91. Two machine-learning eavesdropper
detectors --- a Random Forest and a PyTorch LSTM trained on synthetic QBER
sequences --- demonstrate that data-driven channel monitoring is feasible
even on small key blocks.

The front-end (React + Vite + Tailwind) streams qubit-level events from
the FastAPI backend over WebSockets and presents them as a live photon
grid, real-time QBER charts, decoy-state and finite-key panels, and a
``Channel Status'' badge. A gamified \textbf{Challenge Mode} adds fifteen
procedurally-parameterised missions (Detective: detect Eve from
observations; Engineer: configure QKD to satisfy a constraint) with a
global XP leaderboard, persistent progress and SQLite-backed audit trails.

Experimental results confirm the expected QBER signatures of each attack
type (intercept-resend $\approx 25\%$, weak Eve $\approx 12\%$, smart Eve
tuned to $9\%$, PNS leaving QBER unchanged but collapsing the single-photon
yield $Y_1$ in the decoy analysis), reproduce the $1/\sqrt{n}$ finite-key
penalty, and yield validation accuracies of $0.93$ (Random Forest) and
$0.92$ (LSTM) on held-out runs. Together, these results show that Sequre
is both a pedagogically useful sandbox and a credible experimental platform
for undergraduate-level QKD research.
...
\begin{enumerate}[label=\textbf{C\arabic*.}, leftmargin=*, itemsep=6pt]
  ...
  \item \textbf{Multi-user platform with authenticated session management.}
        ...
        be attributed to authenticated users and enables per-user progress
        tracking across the gamified Challenge Mode, while preserving full
        anonymous guest access for the simulation features.
  ...
  \item \textbf{A configurable research testbed for QKD experimentation.}
        ...
        source type, decoy intensities, error-correction algorithm, detector
        efficiency $\eta_\mathrm{det}$, dark-count rate) as user-controlled
        inputs, together with a reproducibility seed for deterministic runs
        and seven preset scenarios that instantly configure canonical
        experimental setups. The platform is suitable not only for classroom
        demonstration but also as an exploratory tool for studying protocol
        behaviour under varied hardware and adversarial conditions.
  ...
  \item \textbf{Gamified single-player Challenge Mode with global leaderboard.}
        A self-contained \emph{Challenge Mode} page presents 15 procedurally
        parameterised QKD missions across two pedagogical formats.
        \textit{Detective} missions run a hidden simulation and ask the student
        to declare whether Eve is present and which attack she is using, based
        solely on the observable panels (QBER, LSTM probability, decoy-state
        $Y_1$, CHSH $S$). \textit{Engineer} missions present a constrained
        objective (e.g.\ ``achieve $\geq 256$ bits of finite-key output over a
        130\,km link under PNS'') and ask the student to configure the QKD
        parameters to meet it. Each mission is parameterised within a
        difficulty band on every attempt; a global XP leaderboard, per-user
        progress tracking, and targeted post-failure hints complete the
        learning loop. The Challenge Mode reuses the entire simulation and
        visualisation stack without duplication, and is implemented as a
        separate \texttt{/challenge} route with its own authenticated REST API.
...
presentation layers are cleanly separated --- and this modularity has enabled
the integration of decoy-state BB84, CASCADE error correction, an
optical-fibre attenuation model, an adaptive (Smart) Eve, an ML-based
detection layer, three alternative QKD protocols (B92, SARG04, E91 with CHSH
Bell test), an explicit PNS-attack simulator, composable finite-key security
bounds, a deep-learning (LSTM) eavesdropper detector, and a fully gamified
\emph{Challenge Mode} into the same code base without restructuring.
...
limitations, and positions Sequre relative to the state of the art.
...
flag. It does not support multi-user operation, persistent history, or
...
These notebooks are closest in spirit to Sequre: they use the same
Qiskit circuit model, support noise injection, and are intended for
students. However, they are static notebooks rather than a deployable
web application; they do not persist results, stream per-qubit events in
real time, connect to live IBM hardware via a polished interface, or offer
any adversarial simulation, ML analysis, or gamified challenge layer.
...
dimensions most relevant to Sequre's design goals.

\begin{table}[H]
  \centering
  \caption{Comparison of QKD simulation platforms.}
  ...
  \begin{tabular}{@{} l c c c c c @{}}
    ...
    \textbf{Qiskit nb.} \\
    \midrule
    Real hardware execution   & \checkmark & -- & -- & -- & -- \\
    Real-time visualisation   & \checkmark & -- & -- & -- & -- \\
    Multiple Eve models       & \checkmark & limited & -- & -- & -- \\
    Decoy-state / PNS         & \checkmark & -- & -- & -- & -- \\
    Finite-key analysis       & \checkmark & -- & -- & -- & -- \\
    CASCADE error correction  & \checkmark & -- & -- & -- & -- \\
    ML eavesdropper detection & \checkmark & -- & -- & -- & -- \\
    Alternative protocols     & \checkmark & partial & partial & \checkmark & partial \\
    Multi-user / auth         & \checkmark & -- & -- & -- & -- \\
    Challenge Mode / gamification & \checkmark & -- & -- & -- & -- \\
    Browser-accessible        & \checkmark & -- & -- & -- & \checkmark \\
    \bottomrule
  \end{tabular}
\end{table}

The defining differentiator of Sequre is the combination of
...
web interface} and a \emph{gamified learning layer}. No existing open
platform addresses all three simultaneously.
...
This chapter describes the core technologies that underpin the Sequre
...
computing. Sequre uses it to construct per-qubit BB84 circuits: Alice's
...
noisy simulation via the \texttt{NoiseModel} API. Sequre builds a noise
...
For real-hardware execution, Sequre integrates with the
...
\textbf{FastAPI} is a modern, async Python web framework based on
\texttt{asyncio} and the ASGI specification. It was chosen for three
reasons. First, its native \texttt{async/await} support makes it
...
Sequre's FastAPI application exposes two types of endpoints: REST
...
Sequre, the frontend opens a WebSocket to
...
(\texttt{sequre.db}) via the \textbf{SQLAlchemy} ORM. SQLite was selected
over a client-server database (e.g.\ PostgreSQL) because Sequre is
...
\texttt{ALTER TABLE} statements executed at startup, which add new
...
project. Sequre uses Tailwind exclusively for styling: all component
...
Sequre follows a classic three-tier web architecture: a \textbf{React SPA}
...
\begin{figure}[H]
  \centering
  % SCREENSHOT PLACEHOLDER: deployment topology diagram
  % Replace this comment with \includegraphics once the figure is prepared.
  \fbox{\parbox{12cm}{\centering\vspace{2cm}
    \textit{[Figure: system topology — Browser $\leftrightarrow$ Vite/FastAPI $\leftrightarrow$ SQLite; FastAPI $\rightarrow$ IBM Quantum Runtime]}
  \vspace{2cm}}}
  \caption{Sequre deployment topology. The browser communicates with the
...
\begin{enumerate}
  \item The user configures parameters in \texttt{SimulationControls} and
        presses \emph{Run Simulation}.
  \item The \texttt{useSimulationSocket} hook opens a WebSocket to
  ...
  \item The FastAPI handler authenticates the optional session cookie,
        ...
        simulation path (\texttt{\_run\_bb84\_path} or
        \texttt{\_run\_alt\_protocol}).
...
The left-hand \texttt{SimulationControls} panel exposes every free parameter
of the simulation:

\begin{description}
  \item[\textbf{Preset scenarios.}] Seven one-click presets configure all
        ...
        preset per alternative protocol (\textit{B92 lab},
        \textit{SARG04 vs PNS}, \textit{E91 Bell test}). The active preset
...
\begin{figure}[H]
  \centering
  % SCREENSHOT PLACEHOLDER: SimulationControls panel
  \fbox{\parbox{6cm}{\centering\vspace{4cm}
    \textit{[Figure: SimulationControls panel showing preset scenarios,\\
    noise sliders, WCP source, and Eve selector]}
  \vspace{4cm}}}
  \caption{The Sequre configuration panel showing BB84 preset scenarios
...
The core simulation loop in \texttt{api/websocket\_routes.py} processes
qubits sequentially:

\begin{enumerate}
  \item Alice's bits and bases are drawn from \texttt{numpy.random.default\_rng}.
  \item Bob's bases are drawn independently.
  \item If WCP source: \texttt{simulate\_wcp\_pulses()} draws per-pulse
        photon numbers from $\mathrm{Poisson}(\mu)$, applies channel
        transmittance, and returns a \texttt{PulseResult} list. If PNS Eve
        is active, \texttt{apply\_pns\_attack()} modifies the list in-place.
  \item If ideal source with channel distance: \texttt{apply\_channel\_loss()}
...
\begin{figure}[H]
  \centering
  % SCREENSHOT PLACEHOLDER: PhotonGrid with Eve markers
  \fbox{\parbox{14cm}{\centering\vspace{3cm}
    \textit{[Figure: PhotonGrid showing 80 qubit cells; orange-bordered
    cells indicate Eve interception; red cells indicate basis-matched errors]}
  \vspace{3cm}}}
...
\begin{figure}[H]
  \centering
  % SCREENSHOT PLACEHOLDER: LSTM Detection Panel
  \fbox{\parbox{14cm}{\centering\vspace{3cm}
    \textit{[Figure: LstmDetectionPanel showing P(Eve) = 97.4\% for a Weak Eve run,
    probability bar with 0.5 threshold marker, and model metrics]}
  \vspace{3cm}}}
...
\begin{figure}[H]
  \centering
  % SCREENSHOT PLACEHOLDER: Challenge Mode - Level Grid
  \fbox{\parbox{14cm}{\centering\vspace{3.5cm}
    \textit{[Figure: Challenge Mode level grid — 15 mission cards with
    locked/unlocked/completed state, XP reward, difficulty badge, and
    type badge (Detective / Engineer)]}
  \vspace{3.5cm}}}
...
\begin{figure}[H]
  \centering
  % SCREENSHOT PLACEHOLDER: Mission Result Modal
  \fbox{\parbox{10cm}{\centering\vspace{3.5cm}
    \textit{[Figure: MissionResultModal — \textit{Mission complete},
    score 100, +80 XP, verdict breakdown, explanation, progress cells,
...
All results in this chapter were obtained by running Sequre in
...
conditions, BB84 achieved $49.8 \pm 1.2\%$ sifting efficiency --- close
...
\begin{figure}[H]
  \centering
  % SCREENSHOT PLACEHOLDER: QBER Historical Chart
  \fbox{\parbox{14cm}{\centering\vspace{3cm}
    \textit{[Figure: QBERChart showing QBER timeline across $\sim$30 runs;
    runs with Strong Eve clearly above the 11\% red threshold line;
    Weak Eve runs in the 7--10\% range]}
  \vspace{3cm}}}
...
Sequre demonstrates that research-grade QKD analysis --- composable
